    ...
    <div class="bg-base-100 w-full max-w-md rounded-xl shadow-2xl overflow-hidden">
      ...
      <div class="p-6 space-y-6">
        ...
        <div class="form-control">
          <label class="label">
             <span class="label-text text-lg font-bold text-ink">排版方向</span>
          </label>
          <div class="btn-group w-full grid grid-cols-2">
            <button 
              class="btn btn-outline"
              :class="{ 'btn-active bg-cinnabar text-white border-cinnabar hover:bg-red-800': config.layoutDirection === 'vertical' }"
            ...
            <button 
              class="btn btn-outline"
              :class="{ 'btn-active bg-cinnabar text-white border-cinnabar hover:bg-red-800': config.layoutDirection === 'horizontal' }"
          ...
          <div v-if="config.layoutDirection === 'vertical'" class="mt-2 pl-1">
             <label class="label-text text-sm text-stone-500 mr-2">列排列顺序:</label>
             <div class="btn-group btn-group-sm inline-flex align-middle">
               <button 
                 class="btn btn-xs"
                 :class="config.verticalColumnOrder === 'rtl' ? 'btn-active bg-stone-600 text-white' : 'btn-ghost'"
               ...
               <button 
                 class="btn btn-xs"
                 :class="config.verticalColumnOrder === 'ltr' ? 'btn-active bg-stone-600 text-white' : 'btn-ghost'"
        ...
        <div class="form-control">
          <label class="label cursor-pointer justify-start gap-4">
             <span class="label-text text-lg font-bold text-ink">智能格线吸附</span>
             <input type="checkbox" class="toggle toggle-error" v-model="config.smartSnap" />
          </label>
          <p class="text-xs text-stone-500 mt-1 pl-1">
            根据诗词格律自动调整行列数（例如七言诗自动设为8行）
          </p>
        </div>
        
        <!-- Manual Grid Override -->
        <div class="form-control bg-base-100 p-3 rounded-lg border border-base-300">
           <label class="label cursor-pointer justify-start gap-4 pb-0">
             <span class="label-text font-bold text-ink">固定行列 (手动控制)</span>
           ...
           <div v-if="config.fixedGrid.enabled" class="grid grid-cols-2 gap-4 mt-3">
             <div class="form-control">
               <label class="label text-xs text-stone-500 py-1">
             ...
             <div class="form-control">
               ...
               <button class="btn btn-sm btn-disabled btn-ghost border-stone-200">
                 自动计算
               </button>
             </div>
           </div>
        ...
        <div class="space-y-3">
          <label class="label">
             <span class="label-text text-lg font-bold text-ink">边框模式</span>
          </label>
          <div class="btn-group w-full grid grid-cols-3">
            <button 
              class="btn btn-outline"
              :class="{ 'btn-active bg-cinnabar text-white border-cinnabar': config.borderMode === 'full' }"
            ...
            <button 
              class="btn btn-outline"
              :class="{ 'btn-active bg-cinnabar text-white border-cinnabar': config.borderMode === 'lines-only' }"
            ...
            <button 
              class="btn btn-outline"
              :class="{ 'btn-active bg-cinnabar text-white border-cinnabar': config.borderMode === 'none' }"
